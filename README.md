...
Replays now also carry **your mouse**: where the cursor moved and what it clicked, drawn as a pointer over the board. That is the part of a match the markup never shows — the card you picked up, held over a battlefield and put back down, or the long hover before a showdown — and it is worth more on your own replays than anything else in the stream. Two limits worth knowing: it is **your cursor only**, because your opponent's pointer never touches your browser; and **replays recorded before this version have none**, so those play back with no pointer at all rather than one parked in a corner. Movement is sampled ten times a second, and only while the mouse is actually moving; on a match spent constantly moving it the recording grows by up to a tenth, and by rather less than that in practice. One side effect worth expecting: because the cursor now travels, replays show the board's own **hover** styling as it passes — which older recordings, having no cursor to move, never did.
