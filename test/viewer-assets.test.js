...
test("both surfaces draw the replay cursor the same way", () => {
  ...
  const rule = (css, where) => {
    const found = css.match(/\.(?:vr-scale|scale) (\.replayer-mouse[^{]*)\{([^}]*)\}/);
    assert.ok(found, `${where} must style the rrweb cursor - has the override been dropped?`);
    return { suffix: found[1].trim(), body: found[2].trim() };
  };
  const dash = rule(dashboardCss, "dashboard.css");
  const viewer = rule(viewerCss, "viewer.css");
